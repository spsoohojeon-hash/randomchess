...
export default function ResearchDashboard(){
 ...
 async function exportGame(id:string,through:number){setDownload(true);try{let after=-1;const parts:BlobPart[]=[];while(after<through){const p=await api<{events:{seq:number;payload:string}[]}>(`events?id=${encodeURIComponent(id)}&after=${after}`);if(!p.events.length)break;for(const event of p.events){if(event.seq>through)break;const bytes=Uint8Array.from(atob(event.payload),(c:string)=>c.charCodeAt(0));const raw=await new Response(new Blob([bytes]).stream().pipeThrough(new DecompressionStream('gzip'))).text();parts.push(raw+'\n');after=event.seq;}}
  const url=URL.createObjectURL(new Blob(parts,{type:'application/x-ndjson'}));const a=document.createElement('a');a.href=url;a.download=`research-${id}-through-${through}.jsonl`;a.click();setTimeout(()=>URL.revokeObjectURL(url),30000);
 ...
 <div className="research-columns"><section className="research-panel"><div className="research-heading"><h2>현재 대국</h2><span>5초마다 갱신</span></div>{live?<><div className="research-players"><span>백 · {name(live.white_card)}</span><span>흑 · {name(live.black_card)}</span></div><div className="research-board" aria-label="현재 대국 체스판">{(JSON.parse(live.board) as ({color:string;kind:string}|null)[]).map((p,i)=><div key={i} className={(Math.floor(i/8)+i%8)%2?'dark':'light'}>{p&&<span className={p.color==='w'?'white-piece':'black-piece'}>{pieces[p.color+p.kind]}</span>}</div>)}</div><div className="research-game-meta"><strong>{live.ply} 반수 · {live.seq} 행동</strong><span>{live.status==='finished'?(live.winner==='draw'?'무승부':live.winner==='w'?'백 승리':'흑 승리'):live.status==='blocked'?'확인 필요':'진행 중'}</span></div>{live.reason&&<p>{live.reason}</p>}<button disabled={download} onClick={()=>void exportGame(live.id,live.seq)}>{download?'기록 준비 중…':'현재까지 JSONL 받기'}</button><p className="research-muted">실제 상태는 심판·학습 라벨용으로만 저장됩니다. AI는 자기 관측 정보로 판단합니다.</p></>:<p>첫 대국을 기다리는 중입니다.</p>}</section>
