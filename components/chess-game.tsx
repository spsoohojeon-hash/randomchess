...
export default function ChessGame(){
  ...
  const status=!active?"대국을 준비하세요":waiting?"상대의 참가를 기다리는 중":game.result?game.result.winner==="draw"?"무승부":`${sideName(game.result.winner)} 승리`:game.phase==="rescue"?`${sideName(game.pending?.chooser??game.turn)} · 구제 능력을 선택하세요`:game.phase==="reaction"?`${sideName(game.pending?.chooser??game.turn)} · 왕룩을 선택하세요`:game.phase==="choice"?`${sideName(game.pending?.chooser??game.turn)} · 결과를 선택하세요`:game.phase==="duel"?"속전속결 · 가위바위보":`${sideName(game.turn)}의 차례${game.doubleLeft?` · ${game.doubleLeft}회 이동 남음`:""}`;
  ...
  return <div className="app-shell">
    ...
    <Dialog open={active&&!waiting&&game.phase==="rescue"} onOpenChange={()=>{}}><DialogContent className="game-dialog" showCloseButton={false}><DialogTitle>패배 전 능력 사용</DialogTitle><DialogDescription>킹 포획으로 능력을 뽑았습니다. 패배를 확정하기 전에 사용할 능력을 선택하세요.</DialogDescription>{game.pending&&(!online||mySide===game.pending.chooser)?<><div className="choice-buttons">{(game.deathRescues?.[game.pending.chooser]??[]).map(card=><button className="secondary-button" key={card} disabled={busy} onClick={()=>void send({type:"ability",card},game.pending!.chooser)}>{cardInfo(card).name}</button>)}</div><button className="quiet-button" disabled={busy} onClick={()=>void send({type:"acceptDefeat"},game.pending!.chooser)}>사용하지 않고 패배 확정</button></>:<p>상대의 능력 선택을 기다리고 있어요.</p>}</DialogContent></Dialog>
