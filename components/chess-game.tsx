...
import { CARDS, cardInfo, createGame, drawCards, applyAction, movesFor, abilityTargets, abilityError, exorcismTargets, kingScore, sideName, other, kindName, square, isRoyal, generalAssignment, gatlingImpacts, blastArea, normalizeCardPool, handbookDescription, visibleAbilityId } from "@/lib/game";
...
export default function ChessGame(){
  ...
  const submitWithPromotion=(action:Action,side:Side,p:Piece|null)=>{
    const army=game.abilities[side].general;
  ...
  const useAbility=(side:Side)=>{
    ...
    const a=game.abilities[side];
    if(id==="burrow"&&a.burrow){void send({type:"ability"},side);return;}
    if(id==="mounted"&&a.fusion){void send({type:"ability"},side);return;}
    if(id==="armyForward"&&a.uses===0){void send({type:"ability"},side);return;}
    if(id==="necro"){setNecro(side);return;}
    if(["exorcism","spaceTravel","equality","burrow","shiningKnight","armyForward","general","gatling","shallNotPass","mounted"].includes(id)){setStage({side});return;}
    if(id==="kingReturn"){
  ...
  const clickSquare=(i:number)=>{
    ...
    if(stage){
      ...
      if(id==="shallNotPass"){
        setConfirm({title:"능력 발동",description:"선택한 비숍과 같은 세로줄의 상대 기물을 제거합니다. 킹은 면역입니다.",label:"발동",danger:true,run:()=>void send({type:"ability",from:i},stage.side)});return;
      }
      if(id==="exorcism"){
      ...
      submitWithPromotion({type:"ability",from:stage.from,to:i},stage.side,["spaceTravel","shiningKnight"].includes(id)?game.board[stage.from]:null);return;
    }
  ...
  const ammoSides=(["w","b"] as Side[]).filter(s=>active&&s===mySide&&game.abilities[s].id==="gatling");
  const hiddenPieces=(["w","b"] as Side[]).flatMap(s=>s===mySide&&game.abilities[s].burrow?[game.abilities[s].burrow!]:[]);
    ...
    <main className="game-layout">
      <section className="board-section" aria-label="대국">
        ...
        <div className={`board-with-ammo ${ammoSides.length?"has-ammo":""}`}>
          {ammoSides.length>0&&<aside className="ammo-rail" aria-label="내 탄약">{ammoSides.map(s=><div className="ammo-counter" key={s}><span>{online?"탄약":sideName(s)}</span><strong aria-live="polite">{game.abilities[s].ammo??0}<small>/8</small></strong><div className="ammo-rounds" aria-hidden="true">{Array.from({length:8},(_,i)=><i key={i} className={i<(game.abilities[s].ammo??0)?"loaded":""}/>)}</div>{(game.abilities[s].ammo??0)===8&&<span className="ammo-full">범위</span>}</div>)}</aside>}
        <div className="board-frame"><div ref={boardRef} className="chessboard" role="group" aria-label="체스판. 방향키로 칸을 옮기고 엔터로 선택하세요.">
          {visualSquares.map((i,vi)=>{const p=game.board[i],light=(Math.floor(i/8)+i%8)%2===0,highlight=legal.includes(i),royal=p&&isRoyal(game,p)&&p.kind!=="k";return <button key={i} data-square={i} type="button" tabIndex={i===focusSquare?0:-1} aria-label={`${square(i)}${p?` ${sideName(p.color)} ${kindName[p.kind]}`:" 빈칸"}${highlight?" · 선택 가능":""}`} aria-pressed={selected===i} className={`cell ${light?"light":"dark"} ${selected===i?"selected":""} ${lastMove?.from===i||lastMove?.to===i?"last-move":""} ${dangerous.includes(i)?"blast-target":""} ${highlight&&p?"capture-target":""}`} onClick={()=>clickSquare(i)} onKeyDown={e=>{
            ...
            {p&&game.abilities[p.color].general?.id===p.id&&<span className="general-mark" title="장군">★</span>}
            {p&&[game.abilities[p.color].general?.knightId,game.abilities[p.color].general?.bishopId].includes(p.id)&&<span className="general-mark" title="휘하">·</span>}
