...
import { CARDS, cardInfo, createGame, drawCards, applyAction, movesFor, abilityTargets, abilityError, exorcismTargets, threatened, kingScore, sideName, other, kindName, square, isRoyal } from "@/lib/game";
...
export default function ChessGame(){
  ...
  const checked=useMemo(()=>[...threatened(game,"w"),...threatened(game,"b")],[game]);
  const clickSquare=(i:number)=>{
    setFocusSquare(i);if(!active||waiting||busy||game.phase==="over")return;setError("");
    if(game.phase==="reaction"){if(legal.includes(i))void send({type:"reaction",piece:i},game.pending!.chooser);return;}
    if(game.phase!=="play")return;
    if(stage){
  ...
  const cardPanel=(side:Side)=>{
    const a=game.abilities[side],info=cardInfo(a.id),why=abilityError(game,side),canControl=!online||side===mySide;
    ...
    return <article className={`ability-card ${side===game.turn?"current-card":""} ${used?"used-card":""}`} key={side}>
      ...
      {a.id==="equality"&&<div className="ability-stat">캐슬링 성공 <strong>{a.castleCount} / 10</strong></div>}
  ...
  return <div className="app-shell">
    ...
    <main className="game-layout">
      <section className="board-section" aria-label="대국">
        <div className="board-heading"><div><span className="eyebrow">{active?online?"ONLINE MATCH":"LOCAL MATCH":"YOUR NEXT MOVE"}</span><h1>{active?"한 판의 변수":"다음 수는, 예측 불가."}</h1></div><span className="round-badge">{active?`${Math.floor(game.ply/2)+1}번째 턴`:"2인 대전"}</span></div>
        <div className={`player-bar ${active&&game.turn===(flip?"w":"b")?"player-active":""}`}><div className="player-identity"><span className={`player-avatar avatar-${flip?"w":"b"}`}><ChessKing/></span><div><strong>{sideName(flip?"w":"b")}{online&&mySide===(flip?"w":"b")?" · 나":""}</strong><span>{active?cardInfo(game.abilities[flip?"w":"b"].id).name:"능력 추첨 대기"}</span></div></div><div className="captured-pieces" aria-label="잡은 기물">{game.captured[flip?"w":"b"].slice(-10).map((p,i)=><PieceIcon key={i} piece={p} small/>)}</div></div>
        <div className="board-frame"><div ref={boardRef} className="chessboard" role="group" aria-label="체스판. 방향키로 칸을 옮기고 엔터로 선택하세요.">
          {visualSquares.map((i,vi)=>{const p=game.board[i],light=(Math.floor(i/8)+i%8)%2===0,highlight=legal.includes(i),royal=p&&isRoyal(game,p)&&p.kind!=="k";return <button key={i} data-square={i} type="button" tabIndex={i===focusSquare?0:-1} aria-label={`${square(i)}${p?` ${sideName(p.color)} ${kindName[p.kind]}`:" 빈칸"}${highlight?" · 선택 가능":""}`} aria-pressed={selected===i} className={`cell ${light?"light":"dark"} ${selected===i?"selected":""} ${lastMove?.from===i||lastMove?.to===i?"last-move":""} ${checked.includes(i)&&active?"in-danger":""} ${dangerous.includes(i)?"blast-target":""} ${highlight&&p?"capture-target":""}`} onClick={()=>clickSquare(i)} onKeyDown={e=>{
      ...
      <aside className="control-column">
        ...
        {!active?<section className="setup-panel"><div className="panel-heading"><Dices/><span className="eyebrow">NEW GAME</span></div><h2>어떤 한 판을 둘까?</h2><p className="setup-intro">각자 하나의 능력. 승부를 바꾸는 타이밍은 당신의 몫.</p>
    ...
    <footer className="app-footer"><span>RANDOM CHESS <b>02</b></span><p>같은 체스판, 다른 가능성.</p><button onClick={()=>setRules(true)}>플레이 규칙<BookOpen size={14}/></button></footer>
    <Dialog open={rules} onOpenChange={setRules}><DialogContent className="rules-dialog"><DialogTitle>능력 도감 <span className="accent-text">17</span></DialogTitle><DialogDescription>능력은 시작할 때 각자 하나씩 받고 양쪽에 공개됩니다. 랜덤 추첨은 서로 다른 능력을 배정합니다.</DialogDescription><div className="rules-scroll"><div className="basic-rules"><h3>플레이 규칙</h3><p>일반 이동·캐슬링·앙파상·프로모션을 사용합니다. 체크는 경고이며, 체크메이트로 끝나지 않습니다. 상대 킹을 직접 잡거나 능력의 특별 승리 조건을 만족하면 승리합니다. 위험한 칸으로 킹을 움직이는 것도 허용됩니다. 캐슬링은 움직이지 않은 킹·룩과 비어 있는 경로가 필요하며, 킹이 위협받는 칸을 지나갈 수 없습니다.</p><p>능력 설명에 ‘한 턴’이라고 적힌 행동은 일반 이동을 대신합니다. 판을 뒤집어도 차례는 바뀌지 않습니다. 기권하거나 양쪽이 무승부에 동의하면 대국이 끝납니다. 로컬 대국은 이 기기에 저장되고, 온라인 방은 7일간 유지됩니다.</p><p>원본에서 뜻이 엇갈리던 상호작용은 통일했습니다. 더블무브·우주여행으로 왕룩과 여왕 통치의 퀸도 잡을 수 없습니다. 되감기 사용 횟수는 되돌아가지 않습니다. 야생마는 최신 원본 코드의 3×2 이동을 사용합니다.</p></div><div className="rule-cards">{CARDS.map(c=><article key={c.id}><div className="rule-card-heading"><AbilityIcon id={c.id}/><h3>{c.name}</h3><span>{c.classic?"기본":"확장"}</span></div><p>{c.description}</p><small>{modeLabels[c.mode]}</small></article>)}</div></div></DialogContent></Dialog>
    ...
    <Dialog open={active&&!waiting&&game.phase==="choice"} onOpenChange={()=>{}}><DialogContent className="game-dialog" showCloseButton={false}><DialogTitle>{game.pending?cardInfo(game.abilities[game.pending.owner].id).name:""}</DialogTitle><DialogDescription>{game.pending?cardInfo(game.abilities[game.pending.owner].id).description:""}</DialogDescription>{(!online||mySide===game.pending?.chooser)?<><p>{sideName(game.pending!.chooser)}이 승리할 색을 선택하세요.</p><div className="choice-buttons">{(["w","b"] as Side[]).map(s=><button className="secondary-button" key={s} onClick={()=>void send({type:"choice",choice:s},game.pending!.chooser)}>{sideName(s)} 승리</button>)}</div></>:<p>상대의 선택을 기다리고 있어요.</p>}</DialogContent></Dialog>
