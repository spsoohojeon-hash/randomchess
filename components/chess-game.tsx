...
export default function ChessGame(){
  ...
  return <div className="app-shell">
    ...
    <Dialog open={rules} onOpenChange={setRules}><DialogContent className="rules-dialog"><DialogTitle>능력 도감 <span className="accent-text">17</span></DialogTitle><DialogDescription>능력은 시작할 때 각자 하나씩 받습니다. 온라인에서는 자신의 능력만 확인할 수 있습니다. 랜덤 추첨은 서로 다른 능력을 배정합니다.</DialogDescription><div className="rules-scroll"><div className="basic-rules"><h3>플레이 규칙</h3><p>일반 이동·캐슬링·앙파상·프로모션을 사용합니다. 체크는 경고이며, 체크메이트로 끝나지 않습니다. 상대 킹을 직접 잡거나 능력의 특별 승리 조건을 만족하면 승리합니다. 위험한 칸으로 킹을 움직이는 것도 허용됩니다. 캐슬링은 움직이지 않은 킹·룩과 비어 있는 경로가 필요하며, 킹이 위협받는 칸을 지나갈 수 없습니다.</p><p>능력 설명에 ‘한 턴’이라고 적힌 행동은 일반 이동을 대신합니다. 판을 뒤집어도 차례는 바뀌지 않습니다. 기권하거나 양쪽이 무승부에 동의하면 대국이 끝납니다. 로컬 대국은 이 기기에 저장되고, 온라인 방은 7일간 유지됩니다.</p><p>원본에서 뜻이 엇갈리던 상호작용은 통일했습니다. 더블무브·우주여행으로 왕룩과 여왕 통치의 퀸도 잡을 수 없습니다. 되감기 사용 횟수는 되돌아가지 않습니다. 야생마는 최신 원본 코드의 3×2 이동을 사용합니다.</p></div><div className="rule-cards">{CARDS.map(c=><article key={c.id}><div className="rule-card-heading"><AbilityIcon id={c.id}/><h3>{c.name}</h3><span>{c.classic?"기본":"확장"}</span></div><p>{c.description}</p><small>{modeLabels[c.mode]}</small></article>)}</div></div></DialogContent></Dialog>
