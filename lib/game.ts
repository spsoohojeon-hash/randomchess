...
export type CardId = "necro" | "wildHorse" | "spaceTravel" | "doubleMove" | "equality" | "reactionary" | "exorcism" | "kingReturn" | "bombLauncher" | "noThatMove" | "temusanTimeStone" | "extremeEfficiency" | "quickDuel" | "queenRule" | "versatile" | "fiveAhead" | "conscienceTest" | "burrow" | "shiningKnight" | "forwardPawns" | "nothing" | "armyForward" | "general" | "gatling" | "shallNotPass" | "mounted" | "metamon" | "giveMe";
export type Gesture = "rock" | "scissors" | "paper";
export type Piece = { id: string; color: Side; kind: Kind; moved: boolean; form?:"prince"|"emperor"; promoted?:boolean; wildMoved?:boolean };
...
export type Core = {
  ...
  board:(Piece|null)[]; turn:Side; ply:number; abilities:Record<Side,Ability>; extraAbilities?:Record<Side,Ability[]>; deck?:CardId[];
...
const ability=(id:CardId):Ability=>({id,uses:0,active:["versatile","extremeEfficiency","spaceTravel","giveMe"].includes(id),castleCount:0,eligible:true,threats:0,rookId:null,power:null});
export function abilityStates(g:Core,s:Side):Ability[]{return [g.abilities[s],...(g.extraAbilities?.[s]??[])];}
export function abilityState(g:Core,s:Side,id?:CardId):Ability {return (id?abilityStates(g,s).find(a=>a.id===id):g.abilities[s])??g.abilities[s];}
export function hasAbility(g:Core,s:Side,id:CardId):Ability|undefined{return abilityStates(g,s).find(a=>a.id===id);}
...
export function visibleAbilityId(g:Core,side:Side,viewer:Side):CardId|"hidden" {
  ...
  if(a.id==="hidden"||(["fiveAhead","conscienceTest"].includes(a.id)&&!a.revealed))return "hidden";
...
function shuffleCards(ids:CardId[]):CardId[]{
  const out=[...ids];for(let i=out.length-1;i>0;i--){const n=new Uint32Array(1);crypto.getRandomValues(n);const j=n[0]%(i+1);[out[i],out[j]]=[out[j],out[i]];}return out;
}
function applyExtreme(g:Game,s:Side){
  g.board=g.board.map(p=>p?.color===s?null:p);const r=s==="w"?7:0,fallen:Side[]=[];
  for(const [c,kind] of [[4,"k"],[0,"q"],[3,"q"],[7,"q"]] as [number,Kind][]){
    const at=r*8+c,target=take(g,at,s),piece:Piece={id:s+"eff-"+kind+c+"-"+g.revision,color:s,kind,moved:true};g.board[at]=piece;
...
export function createGame(white:CardId="wildHorse",black:CardId="necro",pool:CardPool="all",randomizeDeck=true):Game {
  ...
  for(const s of ["w","b"] as Side[])if(g.abilities[s].id==="metamon"){
    ...
    if(opponent.id==="fiveAhead"||opponent.id==="conscienceTest"){
      g.abilities[s].revealed=true;g.phase="over";g.result={winner:s,reason:"메타몽이 상대의 결말 능력을 간파했습니다."};return g;
    }
    const copy=ability(opponent.id as CardId);copy.source="metamon";copy.revealed=true;g.abilities[s]=copy;
    if(copy.id==="extremeEfficiency")applyExtreme(g,s);
  }
  for(const s of ["w","b"] as Side[])if(["fiveAhead","conscienceTest"].includes(g.abilities[s].id)) {
    g.phase="choice";g.pending={owner:s,chooser:other(s),options:[],card:g.abilities[s].id as CardId};break;
  }
  return g;
}
...
export function isRoyal(g:Core,p:Piece):boolean {
  const queen=hasAbility(g,p.color,"queenRule"),reaction=hasAbility(g,p.color,"reactionary");
  if(queen?.active)return p.kind==="q";
...
export function movesFor(g:Core,from:number,attacks=false):Move[] {
  ...
  const add=(nr:number,nc:number,extra:Partial<Move>={})=>{
    ...
    const to=nr*8+nc,t=g.board[to];if(attacks||!t||t.color!==p.color||!!hasAbility(g,p.color,"gatling")&&p.kind==="q"&&t.kind==="p")out.push({from,to,...extra});
  };
...
export function generalAssignment(g:Core,s:Side):"n"|"b"|null {
  const army=hasAbility(g,s,"general")?.general;
...
function followers(g:Core,s:Side):string[]{
  const army=hasAbility(g,s,"general")?.general;if(!army)return [];
...
export function abilityTargets(g:Core,s:Side,from?:number,card?:CardId):number[] {
  if(g.onlineView){const pos=from===undefined?"start":String(from);return s===g.onlineView.side?g.onlineView.targets[`${card??g.abilities[s].id}:${pos}`]??g.onlineView.targets[pos]??[]:[];}
  const a=abilityState(g,s,card),id=a.id;
  ...
  if(id==="equality"){
    if(from===undefined)return g.board.flatMap((p,i)=>p?.color===s&&abilityTargets(g,s,i,id as CardId).length?[i]:[]);
...
export function abilityError(g:Game,s:Side,card?:CardId):string|null {
  ...
  if(a.id==="armyForward"&&a.uses===1&&abilityTargets(g,s,undefined,a.id).length<2)return "다시 발동하려면 내 룩 2개가 필요합니다.";
  if(["burrow","shiningKnight","shallNotPass","gatling","mounted","general"].includes(a.id)&&!abilityTargets(g,s,undefined,a.id as CardId).length)return a.id==="general"?"장군의 포획 또는 지정 가능한 휘하 기물이 필요합니다.":"현재 사용할 수 있는 기물이 없습니다.";
  if(a.id==="noThatMove")return g.undo.at(-1)?.by===other(s)&&g.lastAction==="move"?null:"상대의 최근 행동이 이동이어야 합니다.";
  if(a.id==="necro"&&(!g.captured[s].length||!abilityTargets(g,s,undefined,a.id).length))return "잡은 적 기물과 킹 주변 빈칸이 필요합니다.";
  if(["spaceTravel","exorcism","equality"].includes(a.id)&&!abilityTargets(g,s,undefined,a.id as CardId).length)return "현재 사용할 수 있는 기물이 없습니다.";
...
function activateDrawnAbility(g:Game,s:Side,a:Ability,dead?:Piece){
  ...
  if(a.id==="extremeEfficiency"){a.active=true;a.revealed=true;applyExtreme(g,s);}
  if(a.id==="reactionary"){
    const king=dead?.color===s&&dead.kind==="k"?dead:g.board.find(p=>p?.color===s&&p.kind==="k");
    const rookReady=(s==="w"?[56,63]:[0,7]).some(i=>{const p=g.board[i];return p?.color===s&&p.kind==="r"&&!p.moved&&p.id===s+square(i);});
    a.eligible=dead?.color===s&&dead.kind==="k"?rookReady:!!king&&!king.moved&&rookReady;
  }
  if(a.id==="metamon"){
    const opponent=g.abilities[other(s)];opponent.revealed=true;
    if(opponent.id==="metamon"){a.revealed=true;return;}
    if(opponent.id==="fiveAhead"||opponent.id==="conscienceTest"){end(g,s,"메타몽이 상대의 결말 능력을 간파했습니다.");return;}
    const copy=ability(opponent.id as CardId);Object.assign(a,copy,{source:"metamon",revealed:true});activateDrawnAbility(g,s,a,dead);
  }
...
function rewardGiveMe(g:Game,owner:Side,p:Piece){
  ...
  if(g.phase!=="over"&&endings.length>=2){end(g,owner,"줄건줘 · 결말 능력 2장을 동시에 뽑았습니다.");return;}
  if(g.phase!=="over"){
    const choice=drawn.find(id=>id==="fiveAhead"||id==="conscienceTest");
    if(choice){g.phase="choice";g.pending={owner,chooser:other(owner),options:[],card:choice};}
  }
}
function take(g:Game,i:number,by:Side){
  const p=g.board[i];
  if(p&&p.color!==by){
...
function onCapture(g:Game,p:Piece,target:Piece){
  const gatling=hasAbility(g,p.color,"gatling");if(gatling&&p.kind==="q"&&target.kind==="p")gatling.ammo=Math.min(8,(gatling.ammo??0)+1);
  if(target.color!==p.color&&hasAbility(g,p.color,"giveMe")&&g.giveMe)g.giveMe[p.color].mark={pieceId:p.id,reply:other(p.color)};
  const a=hasAbility(g,p.color,"general");if(!a||target.color===p.color)return;
  if(!a.general&&p.kind==="p"){a.general={id:p.id,kills:0};a.active=true;a.revealed=true;}
...
function explode(g:Game,at:number,by:Side,fallenQueens:Side[]){
  for(const s of [by,other(by)] as Side[]){const bomb=hasAbility(g,s,"bombLauncher");if(bomb&&!bomb.uses){
    bomb.uses++;bomb.active=true;bomb.revealed=true;const r=Math.floor(at/8),c=at%8;
...
function afterAction(g:Game,by:Side,fallenQueens:Side[]=[],generalMoved=false){
  ...
  if(defender==="b"&&mounted?.fusion&&g.board.filter(p=>p?.color==="b").length===1&&threatened(g,"b").length){
    mounted.threats++;record(g,by,`흑마 위협 ${mounted.threats}/5`,true);
    if(mounted.threats>=5){end(g,"w","흑마 탄 임금님이 홀로 남은 뒤 체크를 5회 받았습니다.");return;}
  }
  g.ply++;
  ...
  if(generalMoved&&g.phase==="play"){
    const army=hasAbility(g,by,"general")?.general;
...
function promote(p:Piece,to:number,kind?:Kind){if(p.kind==="p"&&(Math.floor(to/8)===0||Math.floor(to/8)===7)){assert(kind&&["q","r","b","n"].includes(kind),"프로모션 기물을 선택해 주세요.");p.kind=kind;p.promoted=true;}}
...
export function applyAction(previous:Game,by:Side,action:Action):Game {
  ...
  if(action.type==="reaction"){
    ...
    const reaction=hasAbility(g,by,"reactionary");assert(reaction,"반동분자 능력이 없습니다.");reaction.active=true;reaction.revealed=true;reaction.rookId=g.board[action.piece]!.id;reaction.uses=1;g.pending=null;g.phase="play";record(g,by,`반동분자 · ${square(action.piece)} 왕룩 지정`,true);resolveRoyals(g);g.revision++;return g;
  }
  if(action.type==="choice"){
    assert(g.phase==="choice"&&g.pending?.chooser===by,"결과를 고를 차례가 아닙니다.");
    assert(action.choice==="w"||action.choice==="b","승리할 색을 선택해 주세요.");
    const card=g.pending.card??g.abilities[g.pending.owner].id;end(g,card==="fiveAhead"?other(action.choice):action.choice,"승리 색 선택 결과가 적용되었습니다.");g.revision++;return g;
  }
  if(action.type==="duel"){
  ...
  if(action.type==="ability"){
    const error=abilityError(g,by,action.card);assert(!error,error||"");a.revealed=true;
    ...
    if(a.id==="wildHorse")a.active=true;
      ...
      if(a.burrow){
        ...
        assert(validSquare(action.from)&&abilityTargets(g,by,undefined,a.id as CardId).includes(action.from),"숨길 내 기물을 선택하세요. 킹은 숨길 수 없습니다.");
      ...
      assert(validSquare(action.from)&&abilityTargets(g,by,undefined,a.id as CardId).includes(action.from),"뛰어넘을 내 기물을 선택하세요.");
      assert(validSquare(action.to)&&abilityTargets(g,by,action.from,a.id as CardId).includes(action.to),"상대 기물을 넘어 빈칸에 착지하세요.");
      ...
      assert(validSquare(action.from)&&abilityTargets(g,by,undefined,a.id as CardId).includes(action.from),"장군 또는 지정할 휘하 기물을 선택하세요.");
      ...
      if(assign){
        ...
        assert(validSquare(action.to)&&abilityTargets(g,by,action.from,a.id as CardId).includes(action.to),"위치를 교환할 휘하 기물을 선택하세요.");
        [g.board[action.from],g.board[action.to]]=[g.board[action.to],g.board[action.from]];consumes=true;
      }
    }
    else if(a.id==="gatling"){
      assert(validSquare(action.from)&&abilityTargets(g,by,undefined,a.id as CardId).includes(action.from),"발사할 내 퀸을 선택하세요.");
      ...
      assert(validSquare(action.from)&&abilityTargets(g,by,undefined,a.id as CardId).includes(action.from),"내 비숍을 선택하세요.");
      ...
      assert(validSquare(action.to)&&abilityTargets(g,by,undefined,a.id as CardId).includes(action.to),"내 킹 주변 빈칸에 놓아 주세요.");
      ...
      assert(validSquare(action.from)&&abilityTargets(g,by,undefined,a.id as CardId).includes(action.from),"내 비숍을 선택해 주세요.");
      ...
      assert(validSquare(action.from)&&abilityTargets(g,by,undefined,a.id as CardId).includes(action.from),"상대 진영 코너의 내 기물을 고르세요.");
      assert(validSquare(action.to)&&abilityTargets(g,by,action.from,a.id as CardId).includes(action.to),"이 칸으로는 이동할 수 없습니다.");
      const p=g.board[action.from]!,target=take(g,action.to,by);g.board[action.to]=p;g.board[action.from]=null;markMoved(g,p);promote(p,action.to,action.promotion);if(target){onCapture(g,p,target);explode(g,action.to,by,fallenQueens);}consumes=true;
    }
    else if(a.id==="equality"){
      assert(validSquare(action.from)&&g.board[action.from]?.color===by,"내 기물을 선택해 주세요.");
      assert(validSquare(action.to)&&abilityTargets(g,by,action.from,a.id as CardId).includes(action.to),"사이에 빈칸 2개가 있는 같은 줄의 내 기물이 필요합니다.");
  ...
  const general=hasAbility(g,by,"general"),kingReturn=hasAbility(g,by,"kingReturn");
  ...
  if(!(general?.general?.id===p.id&&general.general.kills>=2))promote(p,move.to,action.promotion);
  ...
  g.ep=null;if(p.kind==="p"&&!(general?.general?.id===p.id&&general.general.kills>=2)&&Math.abs(move.to-move.from)===16)g.ep={target:(move.from+move.to)/2,pawn:move.to,for:other(by)};
  if(target&&target.color!==by)explode(g,move.to,by,fallenQueens);
  if(p.kind==="k"&&kingReturn?.power){kingReturn.power.left--;if(kingReturn.power.left<=0)kingReturn.power=null;}
  ...
  g.undo.push({by,move,before:snapshot});g.undo=g.undo.slice(-4);afterAction(g,by,fallenQueens,general?.general?.id===p.id);
  g.revision=previous.revision+1;return g;
}
export function publicGame(g:Game,viewer?:Side):Game {
  ...
  if(viewer){
    ...
    for(const a of abilityStates(g,viewer)){
      out.onlineView.targets[`${a.id}:start`]=abilityTargets(g,viewer,undefined,a.id as CardId);
      g.board.forEach((p,i)=>{if(p?.color===viewer)out.onlineView!.targets[`${a.id}:${i}`]=abilityTargets(g,viewer,i,a.id as CardId);});
    }
    g.board.forEach((p,i)=>{if(p?.color===viewer)out.onlineView!.moves[i]=movesFor(g,i);});
    const conceal=(a:Ability):Ability=>{
      if(a.id==="fiveAhead"||a.id==="conscienceTest"||(!a.revealed&&!a.uses))return {id:"hidden",uses:0,active:false,castleCount:0,eligible:false,threats:0,rookId:null,power:null};
      const visible=clone(a);delete visible.burrow;delete visible.fusion;delete visible.general;delete visible.ammo;visible.power=null;visible.rookId=null;return visible;
    };
    ...
    out.log=out.log.map(entry=>entry.ability&&(entry.side===opponent&&hiddenStates.length||hiddenStates.some(a=>entry.text.includes(cardInfo(a.id).name))||hiddenStates.some(a=>a.id==="reactionary"&&entry.text.includes("왕룩")||a.id==="mounted"&&entry.text.includes("흑마")))?{n:entry.n,side:entry.side,text:"능력 관련 행동",ability:true}:entry);
    if(out.result&&hiddenStates.some(a=>out.result!.reason.includes(cardInfo(a.id).name)||a.id==="reactionary"&&out.result!.reason.includes("왕룩")||a.id==="mounted"&&out.result!.reason.includes("흑마")))out.result.reason="능력 효과로 대국이 종료되었습니다.";
    if(out.pending){delete out.pending.card;if(out.pending.chooser!==viewer)out.pending.options=[];}
  }
