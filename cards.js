...
export function useCard(cardId, state) {
  ...
  if (cardId === "equality") {
    ...
    return { ok: true, message: "평등국가 발동! 첫 번째 내 기물을 선택하세요." };
  }

  if (cardId === "reactionary") {
    return { ok: false, message: "반동분자는 내 킹이 잡혔을 때 조건부로 발동됩니다." };
  }
  ...
  if (cardId === "necro") {
    ...
    if (state.necroCapturedPieces.length === 0) return { ok: false, message: "아직 부활시킬 잡은 기물이 없습니다." };
